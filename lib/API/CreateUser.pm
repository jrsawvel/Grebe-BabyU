...
#        $sql    .= "                   values ($username, $enc_pass, $email, $userdigest, $datetime,   $ipaddress, $email)";
        ...
        $sql    .= "insert into $dbtable_users( user_name, password,  email,  user_digest, created_date, orig_email)";
        ...
        $sql = "select user_id, user_digest from $dbtable_users where user_name = $username";
        $db->execute($sql);
        return __set_error($self, "system", "Error executing SQL.", $db->errstr) if $db->err;

        if ( $db->fetchrow ) {
            $self->{userid}     = $db->getcol("user_id");
            $self->{userdigest} = $db->getcol("user_digest");
        }
