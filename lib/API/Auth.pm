...
sub valid_user {
    ...
    my $sql = "select user_id, user_name, password, email, user_digest, session_id, created_date, orig_email from $dbtable_users where user_id=$h->{user_id} and user_status='o'";
    $db->execute($sql);
    return 0 if $db->err;
    ...
    if ( $db->fetchrow ) {
        ...
        $hash{user_digest}    = $db->getcol("user_digest");
