...
sub _modify_password {
    ...
    $sqlstr = "select user_id, password, created_date, email, user_digest, orig_email from $dbtable_users where user_id=$userid and user_name=$tmp_username";
    $db->execute($sqlstr);
    Error::report_error("500", "Error executing SQL", $db->errstr) if $db->err;
    ...
    $sqlstr = "update $dbtable_users set password=$new_enc_pass, user_digest=$new_userdigest, session_id=$tmp_new_sessionid where user_id=$userid and user_name=$username and password=$old_enc_pass and user_digest=$old_userdigest";
    $db->execute($sqlstr);
    Error::report_error("500", "Error executing SQL", $db->errstr) if $db->err;
